...
class Cart extends Component {
  ...
  getdata() {
    axios.get("/cart/cartrel").then((res) => {
      ...
      console.log("cart", res.data[0].userid);
      this.setState({
  ...
  getcoupon() {
    axios.get("/coupon/getcoupon").then((res) => {
      console.log("res", res.data.result[0]);
      this.setState({
        coupon: res.data.result[0].promo_code,
      });
    });
  }
  ...
  addcart() {
    ...
    axios
      .post("/payment/order", this.state.collection)
      .then((response) => {
  ...
  render() {
    return (
      <>
        <h1>Cart</h1>
        <hr></hr>
        <h5 className="blink text-center">
          Available Coupon Code "{this.state.coupon}"
        </h5>
